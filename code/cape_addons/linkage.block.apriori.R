...
linkage.blocks.apriori <- function(data.obj, collapse.linked.markers = TRUE, threshold.power = 1, plot.blocks = TRUE){
	
	
	marker.chr <- data.obj$chromosome
	u_chr <- sort(unique(as.numeric(marker.chr)))
	if(u_chr[1] == 0){u_chr <- c(u_chr[-1], 0)}
	used.markers <- colnames(data.obj$geno)
		...
		if(!collapse.linked.markers || length(chr.markers) == 1 || ch == 0){
			...
			if(length(cm.changes) == 0){ #if there are no changes, put the whole chromosome into the block
				...
				num.blocks <- num.blocks + 1
					...
					chr.marker.locale <- which(marker.chr == ch)
