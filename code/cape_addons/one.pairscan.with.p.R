...
one.pairscan.with.p <- function(phenotype.vector, genotype.matrix, int, covar.vector, pairs.matrix, n.perm = 0, verbose = FALSE){
		...
		get.model.results <- function(marker.names, m1, m2, int.term = NULL){
				...
				if(length(use.covars) > 0){
					if(is.null(int.term)){
						...
						model <- lm(phenotype.vector ~ genotype.matrix[,use.covars] + m1 + m2 + int.term, model = FALSE, x = FALSE, y = FALSE, qr = TRUE)	
							}
					}else{
						if(is.null(int.term)){
							model <- lm(phenotype.vector ~ m1 + m2 + m1:m2, model = FALSE, x = FALSE, y = FALSE, qr = TRUE)	
							}else{
							model <- lm(phenotype.vector ~ m1 + m2 + int.term, model = FALSE, x = FALSE, y = FALSE, qr = TRUE)		
							}
						}
				...
				#pull out the p value for the interaction term
				coef.dim <- dim(model.summ$coefficients)
				pval <- model.summ$coefficients[coef.dim[1], coef.dim[2]]

				results <- list(model.effects, model.se, model.cov.results, pval)
				names(results) <- c("model.effects", "model.se", "model.cov", "pval")
				return(results)
				}
		#============================================================================
		...
		all.model.p <- matrix(NA, nrow = n.pairs, ncol = 1) #a table to hold the p value for each interaction term
		...
		for (m in (1:length(pairs.matrix[,1]))){
			...
			if(missing(int)){
				...
				marker.pair.results <- get.model.results(marker.names = markers, m1 = marker1, m2 = marker2, int.term = int)	
				}
			...
			if(length(marker.pair.results) > 0){
				...
				all.model.p[m,] <- marker.pair.results[[4]]
				...
				if(n.perm > 0){
					...
					while(p < n.perm){
						...
						if(missing(int)){
							...
							marker.shuff.results <- get.model.results(markers, marker1.shuff, marker2.shuff, int.term = int)	
							}
			...
			#add the marker pair names to the results tables and name the columns
	      	...
	      	pairs.matrix <- matrix(apply(pairs.matrix, 2, as.numeric), ncol = 2)
			final.effects.table <- cbind(pairs.matrix, all.model.effects)
			final.se.table <- cbind(pairs.matrix, all.model.se)
			final.p.table <- cbind(pairs.matrix, all.model.p)
			...
			phenotype.results <- list(final.effects.table, final.se.table, final.cov.table, final.p.table)
			names(phenotype.results) <- c("pairscan.effects", "pairscan.se", "model.covariance", "model.pvalues")
