...
writeVarInfObj <- function(data.obj, p.or.q = 0.05, plot.all.vals = FALSE, all.markers = FALSE, standardize = TRUE, not.tested.col = "lightgray", show.marker.labels = FALSE, show.chr = TRUE, label.chr = TRUE, scale.effects = c("log10", "sqrt", "none"), pheno.width = 11, covar.width = 11, covar.labels = NULL, phenotype.labels = NULL){
	...
	#also expand the regions where the covariates are
	...
	if(!plot.all.vals){
		...
		sig.neg <- intersect(which(full.inf.mat < 0), which(full.pval.mat < p.or.q))
		sig.pos <- intersect(which(full.inf.mat > 0), which(full.pval.mat < p.or.q))
	...
	if(scale.effects == "sqrt"){
		...
		plot.obj <- list(x = full.inf.mat, min.x = min(full.inf.mat, na.rm = TRUE), max.x = max(full.inf.mat, na.rm = TRUE), main = main, xlab = "Target", ylab = "Source", mark.coords = not.tested.locale, mark.col = not.tested.col, show.labels = show.marker.labels, chromosome.coordinates = chr.boundaries, chr.names = chr.names, show.pheno.labels = TRUE, extra.col.mat = extra.col.mat)
