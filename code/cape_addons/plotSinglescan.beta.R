#This script plots the results of the 1D scan.
#The input is the data.object. The significance
#threshold and 1D results are stored in that
#object.
#The argument plot.label can be used if a
#specific name is to be added to the plot
#filename. This is useful for keeping
#results from different scans separate.



plotSinglescan.beta <- function(data.obj, chr = NULL, traits = NULL, standardized = TRUE, mark.covar = TRUE, mark.chr = TRUE, plot.type = "h", overlay = FALSE, trait.colors = NULL, show.rejected.markers = FALSE, show.selected.markers = FALSE, show.linkage.blocks = FALSE, r2.thresh = 0.8, mark.block.boundaries = FALSE){
	...
	for(p in 1:length(results.to.plot[1,])){
		...
		if(show.linkage.blocks){
			par(xpd = TRUE)
			null <- lapply(blocks, function(bl) points(x = as.numeric(bl), y = rep((min(all.vals)-(max(all.vals)*0.01)), length(bl)), type = "l", lwd = 3, col = "blue"))
			# null <- lapply(blocks, function(bl) points(x = min(as.numeric(bl)), y = (min(all.vals)-(max(all.vals)*0.01)), pch = "(", col = "blue", cex = 0.8))
			# null <- lapply(blocks, function(bl) points(x = max(as.numeric(bl)), y = (min(all.vals)-(max(all.vals)*0.01)), pch = ")", col = "blue", cex = 0.8))
