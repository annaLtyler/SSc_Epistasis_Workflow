#This function plots m12 and m21 using imageWithText
#so we can plot each set of numbers on a different scale
#testing: p.or.q = 0.01; min.std.effect = 0; plot.all.vals = FALSE; all.markers = FALSE; standardize = TRUE; not.tested.col = "lightgray"; show.marker.labels = FALSE; show.chr = TRUE; label.chr = TRUE; scale.effects = c("log10", "sqrt", "none"); pheno.width = 11; covar.width = 11; covar.labels = NULL; phenotype.labels = NULL

plotVariantInfluences.scales <- function(data.obj, p.or.q = 0.05, min.std.effect = 0, plot.all.vals = FALSE, all.markers = FALSE, standardize = TRUE, not.tested.col = "lightgray", show.marker.labels = FALSE, show.chr = TRUE, label.chr = TRUE, scale.effects = c("log10", "sqrt", "none"), pheno.width = 11, covar.width = 11, covar.labels = NULL, phenotype.labels = NULL, pdf.width = 5, pdf.height = 7, pdf.name = "var.inf.separated.pdf"){
	...
	#make class matrices so we can plot negative and positive
	#interactions and main effects on different scales
	
	int.class <- matrix(NA, dim(var.influence.mat)[1], dim(var.influence.mat)[2])
	int.class[which(var.influence.mat < 0)] <- 1 #assign negative interactions to class 1
	int.class[which(var.influence.mat > 0)] <- 2 #assign positive interactions to class 2
	main.class <- matrix(NA, dim(expanded.pheno.mat)[1], dim(expanded.pheno.mat)[2])
	main.class[which(expanded.pheno.mat < 0)] <- 3 #assign negative main effects to class 3
	main.class[which(expanded.pheno.mat > 0)] <- 4 #assign positive main effects to class 4
	...
	full.class.mat <- cbind(int.class, main.class)
	...
	# not.tested.locale <- which(is.na(rotate.mat(full.inf.mat)), arr.ind = TRUE)
	#we're not using myImagePlot, so just get the regular coordinatest
	not.tested.locale <- which(is.na(full.inf.mat))
	
	if(not.tested.col == FALSE || is.na(not.tested.col)){
		not.tested.locale <- NULL
		}
	full.inf.mat[not.tested.locale] <- 1
	full.class.mat[not.tested.locale] <- 5
	...
	if(length(which(na.omit(as.vector(full.inf.mat)) != 0)) == 0){
		...
		#try just with exponential color scale
		steepness = 1
		# pdf(pdf.name, width = pdf.width, height = pdf.height)
		pdf(pdf.name, width = 12, height = 7)
		# imageWithText(mat = full.inf.mat, show.text = FALSE, col.scale = c("blue", "brown", "blue", "brown", "gray"), class.mat = full.class.mat, exp.steepness = steepness, light.dark = "d", color.fun = "exponential")

		layout.mat <- matrix(c(0,1,1,0,3,2,2,2,2,4), byrow = TRUE, nrow = 2)
		# quartz(width = 10, height = 7)
		layout(layout.mat, heights = c(0.11,1), widths = c(rep(0.7, 4), rep(1,2)))

		par(mar = c(0,3,0,3))
		var.influence.color.mat <- var.influence.mat 
		var.influence.color.mat[which(var.pval.mat > p.or.q)] <- NA
		imageWithTextColorbar(var.influence.color.mat, col.scale = c("blue", "brown"), split.at.vals = TRUE, split.points = 0, exp.steepness = steepness, light.dark = "d", color.fun = "exponential", orientation = "h", cex = 2, grad.dir = "ends", axis.line = 0)
		par(mar = c(0,3,0,3))
		imageWithText(mat = var.influence.color.mat, show.text = FALSE, col.scale = c("blue", "brown", "gray"), exp.steepness = steepness, light.dark = "d", color.fun = "exponential", split.at.vals = TRUE, split.points = 0, grad.dir = "ends")

		par(mar = c(0,3,0,3))
		pheno.influence.color.mat <- expanded.pheno.mat
		pheno.influence.color.mat[which(expanded.pheno.pval.mat > p.or.q)] <- NA
		imageWithTextColorbar(pheno.influence.color.mat, col.scale = c("blue", "brown"), split.at.vals = TRUE, split.points = 0, exp.steepness = steepness, light.dark = "d", color.fun = "exponential", orientation = "h", cex = 2, grad.dir = "ends", axis.line = 0)
		par(mar = c(0,3,0,3))
		imageWithText(mat = pheno.influence.color.mat, show.text = FALSE, col.scale = c("blue", "brown", "gray"), exp.steepness = steepness, light.dark = "d", color.fun = "exponential", split.at.vals = TRUE, split.points = 0, grad.dir = "ends")

		plot.new()
		not.tested.mat <- matrix(NA, nrow = nrow(var.influence.color.mat), ncol = ncol(var.influence.color.mat))
		not.tested.mat[which(is.na(var.influence.mat))] <- 1
		par(mar = c(0,3,0,3))
		imageWithText(not.tested.mat, show.text = FALSE, col.scale = "gray", light.dark = "d")
