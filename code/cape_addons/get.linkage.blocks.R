#This function gets linkage blocks
#in the full genome


get.linkage.blocks <- function(data.obj, geno.obj = NULL, collapse.linked.markers = TRUE, threshold.power = 1, plot.blocks = FALSE){
		
	full.geno <- get.geno.with.covar(data.obj, geno.obj, for.pairscan = FALSE)
	
	is.char <- as.logical(is.na(suppressWarnings(as.numeric(colnames(full.geno)[1]))))
			
	
	#find all the chromosomes, and markers
	all.marker.names <- data.obj$marker.names
	all.marker.num <- data.obj$marker.num
	all.marker.chr <- data.obj$chromosome
