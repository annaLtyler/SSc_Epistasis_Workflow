...
plot.LD <- function(data.obj, geno.obj, chr.region, gene.regions = NULL, snp.pos = NULL, snp.name = NULL, ask.before.plotting = FALSE, test.type = c("correlation", "r2")){

	require(igraph)
	
	test.type <- test.type[1]

	if(length(dim(geno.obj)) == 3){
		geno.obj <- geno.obj[,2,]
		}

	locus.dim <- length(dim(geno.obj))
	...
	after.start <- which(data.obj$marker.location >= region.start)
	before.end <- which(data.obj$marker.location <= region.end)
	
	region.locale <- Reduce(intersect, list(chr.locale, after.start, before.end))
	geno.pairs <- pair.matrix(dimnames(geno.obj)[[locus.dim]][region.locale])
	...
	get.r2 <- function(pair.num){
		...
		snp1.geno <- geno.obj[,snp1]
		snp2.geno <- geno.obj[,snp2]
		
		combo <- table(bin.vector(snp1.geno), bin.vector(snp2.geno))
		
		maf1 <- mean(snp1.geno, na.rm = TRUE)
		if(maf1 > 0.5){snp1.geno <- 1 - snp1.geno}

		p1 <- mean(snp1.geno, na.rm  = TRUE)
		q1 <- 1 - p1
		
		maf2 <- mean(snp2.geno, na.rm = TRUE)
		if(maf2 > 0.5){snp2.geno <- 1 - snp2.geno}
		
		p2 <- mean(snp2.geno, na.rm = TRUE)
		q2 <- 1 - p2
	...
	geno.equal <- unlist(lapply_pb(1:nrow(geno.pairs), function(x) cor(geno.obj[,geno.pairs[x,1]], geno.obj[,geno.pairs[x,2]], use = "complete")))
	}else{
